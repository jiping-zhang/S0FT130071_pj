...
<%@ page import="dao.impl.TravelImageDAO" %>
...
<section id="content">
    <%
        ...
        if (imageIDStr==null)
        {
            //System.out.println("imageID not received");
        ...
        else
        {
            ...
            travelImage= TravelImageDAO.getImageByImageID(imageID);
            if (travelImage.getuID()!=travelUser_login.getuID())
            {
                request.getRequestDispatcher("myPhotosServlet").forward(request,response);
                return;
            }
    ...
    %>
    <form name="uploadForm" id="contentForm" action="editServlet" method="post" enctype="multipart/form-data">
        <div class="contRow title">
            上传
        </div>
        <div class="hidden">
            <input type="text" name="imageID" value="<%=travelImage.getImageID()%>">
        </div>
        <div class="contRow">
            <div class="imgContainer" id="imgContainer0">
                <img src="sources/img/<%=travelImage.getPath()%>" class="contImg" id="img0">
            </div>
        ...
        <div class="contRow">
            ...
            <script>
                (function() {
                    ...
                    let options = contentSelect.getElementsByTagName("option");
                    for (let i = 0; i < options.length; i++) {
                        if (options[i].value=='<%=travelImage.getContent()%>')
            ...
            <script>
                (function() {
                    ...
                    let options = countrySelect.getElementsByTagName("option");
                    for (let i = 0; i < options.length; i++) {
                        if (options[i].value=='<%=CountryDAO.getCountryNameByCountryCode(travelImage.getCountryCode())%>')
                            options[i].setAttribute("selected","selected")
                    }
                    setCity();
                    let cityOptions = document.getElementById("city").getElementsByTagName("option")
                    for (let i = 0; i < cityOptions.length; i++) {
                        if (cityOptions[i].value=='<%=CityDAO.getCity(travelImage.getCityCode()).getCityName()%>')
                            cityOptions[i].setAttribute("selected","selected")
                    }
                })()
            </script>
        </div>
        <div class="contRow">
            <p>title</p>
            <br>
            <input type="text" name="title" id="uploadTitle" required value="<%=travelImage.getTitle()%>">
            <br>
            <p>description</p>
            <br>
            <textarea name="description" id="uploadCont"><%=travelImage.getDescription()%></textarea>
        </div>
