...
function getSHApassword(email,password) {
    return sha256_digest(password+email);
}
        ...
        if (rightNickName) {
            if (userName.checkValidity()) {
                if (password.checkValidity()) {
                    if (passwordIntensity.value !== '极弱') {
                        if (password.value === rePassword.value) {
                            if (imgCheckInput.value != 0) {
                                let initialPass=password.value;
                                //console.log(userName+initialPass)
                                password.value=getSHApassword(userName.value,initialPass)
                                signUpForm.submit();
                            }
                            else
                                alert("请先完成滑块验证")
                    ...
                    alert("密码需由6-15位英语字母或数字组成");
            } else
                alert("邮箱名需为有效邮箱地址");

        } else
            alert("昵称需由4-15位 字母或数字或_ 组成")
    }
}();